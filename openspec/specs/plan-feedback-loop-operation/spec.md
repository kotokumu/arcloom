# Plan Feedback Loop Operation

## Purpose

Connect explicit Plan wake-ups, fresh target-bound assessments, and exact result handoff within one caller-scoped operation, without taking authority over external Plan state or downstream decisions.

## Conceptual Model

### Concepts and Authority

| Concept | Meaning and values | Owner |
|---|---|---|
| Plan Feedback Loop Operation | Consumer-visible relationship among explicit wake-ups, published Plan outcomes, and qualifying judgments returned for subsequent consideration; one exact target and one configured destination per caller lifecycle | Plan Controller owns Plan-specific meaning, not Controller scheduling |
| Host invocation | Disposable operational lifetime: Configured, Running, Stopping, Stopped | Host owns intake, transport, evidence, and termination; no Plan judgment |
| Explicit Plan Trigger | An occurrence reduced to an identity-only Request; cause, payload, and count are not Attempt facts | Host trigger source |
| Deliverable Plan Result | Existing exact Assessment from Current Plan Assessed: Complete, Retain, Revise, or Insufficient Information; assessed Plan and optional Proposed Plan remain unchanged | Plan Control owns Assessment meaning; Plan Attempt establishes presence |
| Plan Result Destination | Subsequent continuation, revision, or acceptance consideration for the Assessment; concrete transport is not the destination's meaning or downstream decision | Plan Controller owns the relationship; downstream decision authority stays external |
| Plan Result Delivery | One published assessed Report's exact target/Assessment association with at most one destination invocation; Not Started, In Flight, Delivered, Delivery Failed, Cancelled | Plan-owned delivery preserves qualification and association; Host owns fail-stop lifecycle |
| Processed Report | Passive exact Controller evidence after required handling; Pending, Published, Discarded | Host owns bounded disposable publication, not target authority |

### State and Classification

| Host state | Meaning |
|---|---|
| Configured | Required target, lifecycle, Controller, trigger, consumption, and destination relationships are validated; no trigger accepted |
| Running | Triggers, Report consumption, and qualifying delivery are available |
| Stopping | Cancellation or delivery failure has committed; active boundaries are being cancelled and awaited |
| Stopped | Intake, Controller work, consumption, and delivery have ended |

| Report classification | Delivery classification |
|---|---|
| Current Plan Assessed Completion | Exact existing Assessment qualifies |
| Current Plan Not Established Completion | Observation-only success; no Assessment handoff |
| Attempt Failure | Operational failure; no Assessment handoff |

Delivery disposition and processed publication are independent dimensions. Delivered may coexist with Discarded when cancellation prevents pending evidence publication. Published buffered evidence remains readable after closure. Missing evidence proves neither failure nor absence of a destination effect and grants no replay authority.
...
flowchart LR
    ...
    Delivery -->|exact Assessment through transport| Destination[Subsequent Plan consideration]
    Delivery -->|completion or failure| Host
    Host --> Evidence[Processed Report or terminal failure]
...
- One invocation has one exact target binding, Controller, sole Report consumer, and destination. Triggers cannot substitute another identity or carry target facts.
- Distinct published Reports remain distinct occurrences even when their Assessment values are equal. Publication and delivery are not the same event.
- The existing Assessment is preserved without reconstruction, Snapshot exposure to the recipient, re-evaluation, Authorization, application, or Task execution.
- Runtime scheduling, delivery, and evidence state are disposable; none is authoritative Planning Context.
- Only an External Actor changes authoritative facts between cycles. Later evaluation reacquires current facts rather than replaying earlier results.
- Snapshot and subsequent Delivery Observations retain separate source authority and acquisition evidence; they do not constitute an atomic cross-source snapshot.
- Delivery success and evidence publication have separate commit points. Discarding pending evidence cannot reverse an external effect or require a reader to resume.

### References

- [related] `reconciliation-control-loop` owns identity, Request, Controller, Completion, Failure, and Report.
- [related] `plan-reconciliation-loop` owns fresh evaluation and its two successful branches.
- [related] `plan-control` owns Assessment, assessed Plan, and Proposed Plan meaning.
- [decision] `ARCHITECTURE.md` owns Host and semantic destination boundaries.
