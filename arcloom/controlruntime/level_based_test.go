...
func TestControllerAttemptReacquiresCurrentFacts(t *testing.T) {
	...
	current.Store(1)
	attempt := controlruntime.Attempt[int64](func(context.Context, controlruntime.TargetIdentity) (int64, controlruntime.Directive, error) {
		return current.Load(), controlruntime.AwaitAnotherRequest(), nil
	})
	controller := must(controlruntime.Start(ctx, 1, attempt))
	target := must(controlruntime.NewTargetIdentity("test", "level"))
