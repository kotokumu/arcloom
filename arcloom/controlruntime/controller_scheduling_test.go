...
func TestControllerSchedulingMixedRequestStress(t *testing.T) {
	...
	started := make(chan controlruntime.TargetIdentity, 4)
	var globalActive atomic.Int64
	var maximumObserved atomic.Int64
	var stateMu sync.Mutex
	activeByTarget := map[controlruntime.TargetIdentity]bool{}
	overlapped := false
	attempt := controlruntime.Attempt[string](func(ctx context.Context, target controlruntime.TargetIdentity) (string, controlruntime.Directive, error) {
		stateMu.Lock()
	...
	controller := must(controlruntime.Start(ctx, 4, attempt))
	targets := make([]controlruntime.TargetIdentity, 8)
	for index := range targets {
		targets[index] = must(controlruntime.NewTargetIdentity("stress", string(rune('a'+index))))
	}
	for index := range 4 {
...
func TestControllerSchedulingDuplicatePendingRequestsCoalesce(t *testing.T) {
	...
	attempt := controlruntime.Attempt[string](func(ctx context.Context, target controlruntime.TargetIdentity) (string, controlruntime.Directive, error) {
		if target.Key() == "blocker" {
			...
			case <-releaseBlocker:
			case <-ctx.Done():
				return "", controlruntime.Directive{}, ctx.Err()
			}
		} else {
	...
	blocker := must(controlruntime.NewTargetIdentity("test", "blocker"))
	target := must(controlruntime.NewTargetIdentity("test", "target"))
...
func TestControllerSchedulingCoincidentCompletionAndRequestPreservesLaterAttempt(t *testing.T) {
	for iteration := range 20 {
		...
		attempt := controlruntime.Attempt[int64](func(ctx context.Context, _ controlruntime.TargetIdentity) (int64, controlruntime.Directive, error) {
			call := calls.Add(1)
			started <- struct{}{}
			if call == 1 {
				select {
				case <-gate:
				case <-ctx.Done():
					return 0, controlruntime.Directive{}, ctx.Err()
				}
			}
			return call, controlruntime.AwaitAnotherRequest(), nil
		})
		controller := must(controlruntime.Start(ctx, 1, attempt))
		target := must(controlruntime.NewTargetIdentity("test", "coincident"))
		if err := controller.Request(context.Background(), target); err != nil {
...
func TestControllerSchedulingDistinctTargetsProgressWithinBound(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	started := make(chan controlruntime.TargetIdentity, 2)
	release := make(chan struct{})
	attempt := controlruntime.Attempt[string](func(ctx context.Context, target controlruntime.TargetIdentity) (string, controlruntime.Directive, error) {
		started <- target
		select {
	...
	controller := must(controlruntime.Start(ctx, 2, attempt))
...
func TestControllerSchedulingActiveRequestIsAcceptedAndRunsLater(t *testing.T) {
	...
	call := 0
	attempt := controlruntime.Attempt[int](func(ctx context.Context, target controlruntime.TargetIdentity) (int, controlruntime.Directive, error) {
		call++
		current := call
		started <- current
		if current == 1 {
		...
		return current, controlruntime.AwaitAnotherRequest(), nil
	})
	controller := must(controlruntime.Start(ctx, 1, attempt))
	target := must(controlruntime.NewTargetIdentity("test", "same"))
	if err := controller.Request(context.Background(), target); err != nil {
		t.Fatalf("first Request() error = %v", err)
	}
	select {
