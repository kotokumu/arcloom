...
func TestPlanAttemptResolvesDistinctTargetBindings(t *testing.T) {
	firstIdentity := must(reconciliationcontrol.NewTargetIdentity("plan", "first"))
	secondIdentity := must(reconciliationcontrol.NewTargetIdentity("plan", "second"))
	...
	firstTarget := must(planreconciliation.NewPlanTarget(firstIdentity, func(context.Context) (plansnapshot.Snapshot, error) { return firstSnapshot, nil }, func(context.Context) (string, error) { return "first delivery", nil }))
	secondTarget := must(planreconciliation.NewPlanTarget(secondIdentity, func(context.Context) (plansnapshot.Snapshot, error) { return secondSnapshot, nil }, func(context.Context) (string, error) { return "second delivery", nil }))
	attempt := must(planreconciliation.NewAttempt("plan", func(_ context.Context, requested reconciliationcontrol.TargetIdentity) (planreconciliation.PlanTarget[string], error) {
		...
		return planreconciliation.PlanTarget[string]{}, errors.New("not configured")
	...
	for _, identity := range []reconciliationcontrol.TargetIdentity{firstIdentity, secondIdentity} {
...
func TestPlanAttemptControllerReentryAlwaysObservesFreshFacts(t *testing.T) {
	identity := must(reconciliationcontrol.NewTargetIdentity("plan", "reentry"))
	...
	target := must(planreconciliation.NewPlanTarget(identity, func(context.Context) (plansnapshot.Snapshot, error) {
		observation.Add(1)
	...
	attempt := must(planreconciliation.NewAttempt("plan", func(context.Context, reconciliationcontrol.TargetIdentity) (planreconciliation.PlanTarget[struct{}], error) {
		return target, nil
