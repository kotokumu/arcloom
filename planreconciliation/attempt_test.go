package planreconciliation_test
...
import (
	...
	"github.com/kotokumu/arcloom/planreconciliation"
...
func TestPlanAttemptSetupValidation(t *testing.T) {
	identity := must(reconciliationcontrol.NewTargetIdentity("plan", "one"))
	observer := plansnapshot.Observer(func(context.Context) (plansnapshot.Snapshot, error) { return plansnapshot.Snapshot{}, nil })
	delivery := planreconciliation.DeliveryObserver[string](func(context.Context) (string, error) { return "", nil })
	...
	resolver := planreconciliation.TargetResolver[string](func(context.Context, reconciliationcontrol.TargetIdentity) (planreconciliation.PlanTarget[string], error) {
		return planreconciliation.NewPlanTarget(identity, observer, delivery)
	})
	...
	targetTests := []struct {
		...
		delivery planreconciliation.DeliveryObserver[string]
	}{
	...
	for _, tt := range targetTests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := planreconciliation.NewPlanTarget(tt.identity, tt.observer, tt.delivery)
			if !errors.Is(err, planreconciliation.ErrInvalidPlanTarget) {
	...
	attemptTests := []struct {
		name     string
		kind     string
		resolver planreconciliation.TargetResolver[string]
		assessor plancontrol.Assessor[string]
		want     error
	}{
		{name: "invalid kind", kind: " plan", resolver: resolver, assessor: assessor, want: planreconciliation.ErrInvalidTargetKind},
		{name: "nil resolver", kind: "plan", assessor: assessor, want: planreconciliation.ErrInvalidTargetResolver},
		{name: "nil assessor", kind: "plan", resolver: resolver, want: planreconciliation.ErrInvalidAssessor},
	}
	for _, tt := range attemptTests {
		t.Run(tt.name, func(t *testing.T) {
			attempt, err := planreconciliation.NewAttempt(tt.kind, tt.resolver, tt.assessor)
...
func TestPlanAttemptWithoutCurrentPlanIsSuccessfulObservationOnly(t *testing.T) {
	identity := must(reconciliationcontrol.NewTargetIdentity("plan", "without-current"))
	...
	deliveryCalls := 0
	assessorCalls := 0
	target := must(planreconciliation.NewPlanTarget(identity,
	...
	attempt := must(planreconciliation.NewAttempt("plan",
		func(_ context.Context, requested reconciliationcontrol.TargetIdentity) (planreconciliation.PlanTarget[string], error) {
	...
	if result.Kind() != planreconciliation.CurrentPlanNotEstablished {
	...
	if directive.Kind() != reconciliationcontrol.AwaitRequest {
...
func TestPlanAttemptWithCurrentPlanAssessesInOrder(t *testing.T) {
	...
	identity := must(reconciliationcontrol.NewTargetIdentity("plan", "with-current"))
	tests := []struct {
		name    string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			attempt := must(planreconciliation.NewAttempt("plan",
				func(context.Context, reconciliationcontrol.TargetIdentity) (planreconciliation.PlanTarget[string], error) {
					return target, nil
				},
			...
			if result.Kind() != planreconciliation.CurrentPlanAssessed {
				t.Errorf("AttemptResult Kind = %v", result.Kind())
			}
...
func TestPlanAttemptNormalizesBindingAndDeliveryFailures(t *testing.T) {
	identity := must(reconciliationcontrol.NewTargetIdentity("plan", "failure"))
	other := must(reconciliationcontrol.NewTargetIdentity("plan", "other"))
	...
	tests := []struct {
		name     string
		request  reconciliationcontrol.TargetIdentity
		resolver planreconciliation.TargetResolver[string]
		wantCode planreconciliation.FailureCode
	}{
		{name: "wrong kind", request: must(reconciliationcontrol.NewTargetIdentity("other", "failure")), resolver: func(context.Context, reconciliationcontrol.TargetIdentity) (planreconciliation.PlanTarget[string], error) {
			t.Fatal("resolver called")
			return planreconciliation.PlanTarget[string]{}, nil
		}, wantCode: planreconciliation.TargetBindingUnavailable},
		{name: "zero binding without resolver error", request: identity, resolver: func(context.Context, reconciliationcontrol.TargetIdentity) (planreconciliation.PlanTarget[string], error) {
			return planreconciliation.PlanTarget[string]{}, nil
		}, wantCode: planreconciliation.TargetBindingUnavailable},
		{name: "resolver error", request: identity, resolver: func(context.Context, reconciliationcontrol.TargetIdentity) (planreconciliation.PlanTarget[string], error) {
			return planreconciliation.PlanTarget[string]{}, providerErr
		}, wantCode: planreconciliation.TargetBindingUnavailable},
		{name: "mismatched binding", request: identity, resolver: func(context.Context, reconciliationcontrol.TargetIdentity) (planreconciliation.PlanTarget[string], error) {
			return planreconciliation.NewPlanTarget(other, func(context.Context) (plansnapshot.Snapshot, error) { return snapshot, nil }, func(context.Context) (string, error) { return "", nil })
		}, wantCode: planreconciliation.TargetBindingUnavailable},
		{name: "delivery error", request: identity, resolver: func(context.Context, reconciliationcontrol.TargetIdentity) (planreconciliation.PlanTarget[string], error) {
			return planreconciliation.NewPlanTarget(identity, func(context.Context) (plansnapshot.Snapshot, error) { return snapshot, nil }, func(context.Context) (string, error) { return "", providerErr })
		}, wantCode: planreconciliation.DeliveryObservationUnavailable},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			attempt := must(planreconciliation.NewAttempt("plan", tt.resolver, validAssessor))
			result, directive, err := attempt(context.Background(), tt.request)
			var failure *planreconciliation.FailureError
...
func TestPlanAttemptPreservesSnapshotAndAssessmentFailures(t *testing.T) {
	identity := must(reconciliationcontrol.NewTargetIdentity("plan", "owned-failures"))
	current := must(plan.New("current", must(plan.NewGoal("Deliver the goal")), []plan.AcceptanceCondition{must(plan.NewAcceptanceCondition("The outcome is accepted"))}, nil, nil))
	snapshot := must(plansnapshot.New(current, must(plansnapshot.CompleteProgress(plansnapshot.Open, nil))))
	tests := []struct {
		name                string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			target := must(planreconciliation.NewPlanTarget(identity, tt.observer, func(context.Context) (string, error) { return "delivery", nil }))
			attempt := must(planreconciliation.NewAttempt("plan", func(context.Context, reconciliationcontrol.TargetIdentity) (planreconciliation.PlanTarget[string], error) {
				return target, nil
			}, tt.assessor))
			result, directive, err := attempt(context.Background(), identity)
			if result.Kind() != 0 || directive.Kind() != 0 {
...
func TestPlanAttemptCallerCancellationWinsAtEveryBoundary(t *testing.T) {
	identity := must(reconciliationcontrol.NewTargetIdentity("plan", "cancellation"))
	current := must(plan.New("current", must(plan.NewGoal("Deliver the goal")), []plan.AcceptanceCondition{must(plan.NewAcceptanceCondition("The outcome is accepted"))}, nil, nil))
	snapshot := must(plansnapshot.New(current, must(plansnapshot.CompleteProgress(plansnapshot.Open, nil))))
	validAssessor := plancontrol.Assessor[string](func(context.Context, plan.Plan, string) (plancontrol.AssessorResponse, error) {
		return plancontrol.AssessorResponse{Claims: []plancontrol.Outcome{plancontrol.Complete}}, nil
	...
	t.Run("nil context", func(t *testing.T) {
		...
		if !errors.Is(err, reconciliationcontrol.ErrInvalidContext) || result.Kind() != 0 || directive.Kind() != 0 {
	...
	t.Run("after resolver", func(t *testing.T) {
		ctx, cancel := context.WithCancel(context.Background())
		observerCalls := 0
		target := must(planreconciliation.NewPlanTarget(identity, func(context.Context) (plansnapshot.Snapshot, error) { observerCalls++; return snapshot, nil }, func(context.Context) (string, error) { return "", nil }))
		attempt := must(planreconciliation.NewAttempt("plan", func(context.Context, reconciliationcontrol.TargetIdentity) (planreconciliation.PlanTarget[string], error) {
			cancel()
	...
	t.Run("during resolver", func(t *testing.T) {
		...
		attempt := must(planreconciliation.NewAttempt("plan", func(ctx context.Context, _ reconciliationcontrol.TargetIdentity) (planreconciliation.PlanTarget[string], error) {
			close(started)
			<-ctx.Done()
			return planreconciliation.PlanTarget[string]{}, errors.New("resolver detail")
		}, validAssessor))
	...
	t.Run("after snapshot", func(t *testing.T) {
		ctx, cancel := context.WithCancel(context.Background())
		deliveryCalls := 0
		target := must(planreconciliation.NewPlanTarget(identity, func(context.Context) (plansnapshot.Snapshot, error) { cancel(); return snapshot, nil }, func(context.Context) (string, error) { deliveryCalls++; return "", nil }))
		attempt := must(planreconciliation.NewAttempt("plan", func(context.Context, reconciliationcontrol.TargetIdentity) (planreconciliation.PlanTarget[string], error) {
			return target, nil
		}, validAssessor))
		result, directive, err := attempt(ctx, identity)
	...
	t.Run("during snapshot", func(t *testing.T) {
		ctx, cancel := context.WithCancel(context.Background())
		started := make(chan struct{})
		target := must(planreconciliation.NewPlanTarget(identity, func(ctx context.Context) (plansnapshot.Snapshot, error) {
			close(started)
			<-ctx.Done()
			return plansnapshot.Snapshot{}, plansnapshot.UnavailableObservationFailure()
	...
	t.Run("after delivery", func(t *testing.T) {
		ctx, cancel := context.WithCancel(context.Background())
		assessorCalls := 0
		target := must(planreconciliation.NewPlanTarget(identity, func(context.Context) (plansnapshot.Snapshot, error) { return snapshot, nil }, func(context.Context) (string, error) { cancel(); return "delivery", errors.New("delivery detail") }))
		attempt := must(planreconciliation.NewAttempt("plan", func(context.Context, reconciliationcontrol.TargetIdentity) (planreconciliation.PlanTarget[string], error) {
			return target, nil
		}, func(context.Context, plan.Plan, string) (plancontrol.AssessorResponse, error) {
			assessorCalls++
	...
	t.Run("during delivery", func(t *testing.T) {
		ctx, cancel := context.WithCancel(context.Background())
		started := make(chan struct{})
		target := must(planreconciliation.NewPlanTarget(identity, func(context.Context) (plansnapshot.Snapshot, error) { return snapshot, nil }, func(deliveryCtx context.Context) (string, error) {
			if deliveryCtx != ctx {
	...
	t.Run("after assessment", func(t *testing.T) {
		ctx, cancel := context.WithCancel(context.Background())
		target := must(planreconciliation.NewPlanTarget(identity, func(context.Context) (plansnapshot.Snapshot, error) { return snapshot, nil }, func(context.Context) (string, error) { return "delivery", nil }))
		attempt := must(planreconciliation.NewAttempt("plan", func(context.Context, reconciliationcontrol.TargetIdentity) (planreconciliation.PlanTarget[string], error) {
			return target, nil
		}, func(context.Context, plan.Plan, string) (plancontrol.AssessorResponse, error) {
			cancel()
