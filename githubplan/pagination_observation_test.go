...
func TestPaginationPublicResultAndRequestMatrix(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "milestone multi-page complete", representation: githubplan.MilestoneRepresentation, pageBodies: []string{`[{"id":7,"title":"First"},{"id":8,"title":"Second"}]`, `[{"id":9,"title":"Third"}]`}, links: []string{`<` + milestonePage2 + `>; rel="next"`, ""}, expectedTasks: []string{"First", "Second", "Third"}, wantDetermination: planrepresentation.Satisfied, wantRequests: 3, wantLastURL: milestonePage2},
		{name: "issue multi-page complete", representation: githubplan.IssueRepresentation, pageBodies: []string{`[{"id":7,"title":"First"}]`, `[{"id":9,"title":"Third"}]`}, links: []string{`<` + issuePage2 + `>; rel="next"`, ""}, expectedTasks: []string{"First", "Third"}, wantDetermination: planrepresentation.Satisfied, wantRequests: 3, wantLastURL: issuePage2},
		{name: "later page failure preserves coherent member", representation: githubplan.MilestoneRepresentation, pageBodies: []string{`[{"id":7,"title":"First"}]`, `provider detail`}, pageStatuses: []int{http.StatusOK, http.StatusInternalServerError}, links: []string{`<` + milestonePage2 + `>; rel="next"`, ""}, expectedTasks: []string{"First"}, wantDetermination: planrepresentation.Undecidable, wantUnavailable: []planrepresentation.LocationKind{planrepresentation.TaskCollectionLocation}, wantRequests: 3},
		{name: "same id same title is one incomplete member", representation: githubplan.IssueRepresentation, pageBodies: []string{`[{"id":7,"title":"A"}]`, `[{"id":7,"title":"A"}]`}, links: []string{`<` + issuePage2 + `>; rel="next"`, ""}, expectedTasks: []string{"A"}, wantDetermination: planrepresentation.Undecidable, wantUnavailable: []planrepresentation.LocationKind{planrepresentation.TaskCollectionLocation}, wantRequests: 3},
		{name: "same id conflict A A B", representation: githubplan.MilestoneRepresentation, pageBodies: []string{`[{"id":7,"title":"A"}]`, `[{"id":7,"title":"A"}]`, `[{"id":7,"title":"B"}]`}, links: []string{`<` + milestonePage2 + `>; rel="next"`, `<https://api.github.com/repos/owner/repo/issues?milestone=42&page=3&per_page=100&state=all>; rel="next"`, ""}, wantDetermination: planrepresentation.Undecidable, wantUnavailable: []planrepresentation.LocationKind{planrepresentation.TaskCollectionLocation}, wantRequests: 4},
		...
		{name: "same id conflict reversed page order", representation: githubplan.IssueRepresentation, pageBodies: []string{`[{"id":7,"title":"B"}]`, `[{"id":7,"title":"A"}]`, `[{"id":7,"title":"A"}]`}, links: []string{`<` + issuePage2 + `>; rel="next"`, `<https://api.github.com/repos/owner/repo/issues/42/sub_issues?page=3&per_page=100>; rel="next"`, ""}, wantDetermination: planrepresentation.Undecidable, wantUnavailable: []planrepresentation.LocationKind{planrepresentation.TaskCollectionLocation}, wantRequests: 4},
		{name: "distinct ids equal title are Plan duplicate violation", representation: githubplan.MilestoneRepresentation, pageBodies: []string{`[{"id":7,"title":"Same"}]`, `[{"id":8,"title":"Same"}]`}, links: []string{`<` + milestonePage2 + `>; rel="next"`, ""}, expectedTasks: []string{"Same"}, wantDetermination: planrepresentation.NotSatisfied, wantViolation: plan.DuplicateTask, wantRequests: 3},
		{name: "valid next among other relations", representation: githubplan.IssueRepresentation, pageBodies: []string{`[{"id":7,"title":"First"}]`, `[]`}, links: []string{`<` + issuePage2 + `>; rel="last", <` + issuePage2 + `>; rel="next"`, ""}, expectedTasks: []string{"First"}, wantDetermination: planrepresentation.Satisfied, wantRequests: 3, wantLastURL: issuePage2},
		{name: "duplicate rel next", representation: githubplan.MilestoneRepresentation, pageBodies: []string{`[{"id":7,"title":"First"}]`}, links: []string{`<` + milestonePage2 + `>; rel="next", <` + milestonePage2 + `>; rel="next"`}, expectedTasks: []string{"First"}, wantDetermination: planrepresentation.Undecidable, wantUnavailable: []planrepresentation.LocationKind{planrepresentation.TaskCollectionLocation}, wantRequests: 2},
		{name: "milestone 41", representation: githubplan.MilestoneRepresentation, pageBodies: []string{`[{"id":7,"title":"First"}]`}, links: []string{`<https://api.github.com/repos/owner/repo/issues?milestone=41&page=2&per_page=100&state=all>; rel="next"`}, expectedTasks: []string{"First"}, wantDetermination: planrepresentation.Undecidable, wantUnavailable: []planrepresentation.LocationKind{planrepresentation.TaskCollectionLocation}, wantRequests: 2},
		...
		{name: "duplicate milestone", representation: githubplan.MilestoneRepresentation, pageBodies: []string{`[{"id":7,"title":"First"}]`}, links: []string{`<` + milestonePage2 + `&milestone=42>; rel="next"`}, expectedTasks: []string{"First"}, wantDetermination: planrepresentation.Undecidable, wantUnavailable: []planrepresentation.LocationKind{planrepresentation.TaskCollectionLocation}, wantRequests: 2},
		{name: "missing state", representation: githubplan.MilestoneRepresentation, pageBodies: []string{`[{"id":7,"title":"First"}]`}, links: []string{`<https://api.github.com/repos/owner/repo/issues?milestone=42&page=2&per_page=100>; rel="next"`}, expectedTasks: []string{"First"}, wantDetermination: planrepresentation.Undecidable, wantUnavailable: []planrepresentation.LocationKind{planrepresentation.TaskCollectionLocation}, wantRequests: 2},
		{name: "duplicate state", representation: githubplan.MilestoneRepresentation, pageBodies: []string{`[{"id":7,"title":"First"}]`}, links: []string{`<` + milestonePage2 + `&state=all>; rel="next"`}, expectedTasks: []string{"First"}, wantDetermination: planrepresentation.Undecidable, wantUnavailable: []planrepresentation.LocationKind{planrepresentation.TaskCollectionLocation}, wantRequests: 2},
		{name: "missing per page", representation: githubplan.IssueRepresentation, pageBodies: []string{`[{"id":7,"title":"First"}]`}, links: []string{`<https://api.github.com/repos/owner/repo/issues/42/sub_issues?page=2>; rel="next"`}, expectedTasks: []string{"First"}, wantDetermination: planrepresentation.Undecidable, wantUnavailable: []planrepresentation.LocationKind{planrepresentation.TaskCollectionLocation}, wantRequests: 2},
		...
		{name: "additional selection query", representation: githubplan.IssueRepresentation, pageBodies: []string{`[{"id":7,"title":"First"}]`}, links: []string{`<` + issuePage2 + `&labels=bug>; rel="next"`}, expectedTasks: []string{"First"}, wantDetermination: planrepresentation.Undecidable, wantUnavailable: []planrepresentation.LocationKind{planrepresentation.TaskCollectionLocation}, wantRequests: 2},
...
func TestPagination101ResourcesIncludesLaterPage(t *testing.T) {
	...
	if result.Determination() != planrepresentation.Satisfied || len(result.Differences()) != 0 || len(result.UnavailableInformation()) != 0 {
